...
def print_recent_repositories(events):
    ...
    #         repos[repo_name] = {'api':api_github(event['repo']['url']), 'events': []}
    #     repo = repos[repo_name]
    #     if event['type'] in ['CreateEvent', 'DeleteEvent']:
    #         change = event['type'].replace('Event','') + 'd'
    #         eventType = event['payload']['ref_type'].capitalize()
    #         if eventType in ['Tag', 'Branch']:
    #             repo['events'].append(f"{eventType} {change}: <code>{event['payload']['ref']}</code>")
    ...
    #         if event['payload']['action'] in ['opened', 'closed', 'reopened', 'edited']:
